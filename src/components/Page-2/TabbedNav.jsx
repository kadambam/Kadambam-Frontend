...
const TabbedNav = () => {
  ...
  return (
    <section className="relative bg-white py-8 px-4 sm:px-6 md:px-20">
      {/* Title Section */}
      <motion.div
      ...
      >
        <h2 className="text-sm font-normal text-gray-700" style={{ fontFamily: "Poppins" }}>
          One-stop shop for all things
        </h2>

        <h1 className="text-xl sm:text-2xl font-medium mt-2 leading-tight">
          <span className="relative inline-block">
            <span className="text-red-500">Architectural Services</span>
            <span className="absolute left-1/2 transform -translate-x-1/2 bottom-[-4px] w-8 sm:w-12 h-[2px] bg-red-500" />
          </span>{" "}
          For Your{" "}
          <span className="inline-block relative w-[100px] sm:w-[120px] h-[30px] sm:h-[35px] overflow-hidden align-middle">
            <AnimatePresence mode="wait">
              <motion.span
                ...
                className="absolute left-0 right-0 text-green-500 text-center w-full text-sm sm:text-base"
              >
                {changingWord}
              </motion.span>
            </AnimatePresence>
      ...
      <div className="relative flex overflow-x-auto pb-2 -mb-2">
        <div className="flex justify-start space-x-4 md:space-x-8">
          {tabs.map((tab, index) => (
            <button
              key={index}
              className={`relative py-2 px-3 sm:px-4 text-base sm:text-lg font-medium transition ${activeTab === index ? "text-black" : "text-gray-500 hover:text-gray-800"
                }`}
      ...
      {/* Animated Content Section - Stacked Layout */}
      <motion.div
        key={activeTab}
        ...
        className="mt-6 bg-gray-50 p-4 rounded-xl shadow-lg flex flex-col items-center gap-6"
      >
        {/* Big Image */}
        <div className="w-full flex justify-center items-center">
          <motion.img
            ...
            transition={{ duration: 0.4 }}
            className="w-full h-auto max-h-[300px] rounded-md object-contain"
          />
        </div>
        ...
        <div className="text-left w-full">
          ...
          <p className="text-gray-700 mt-3 leading-relaxed text-sm">{tabs[activeTab].content}</p>
          <button
            className="flex items-center gap-2 px-4 py-2 mt-4 bg-white text-black border border-gray-400 rounded-lg hover:bg-gray-800 hover:text-white transition text-sm"
      ...
      <a
        ...
        className="fixed bottom-3 right-3 bg-green-500 text-white p-2 rounded-full shadow-lg hover:bg-green-600 transition z-50"
      >
